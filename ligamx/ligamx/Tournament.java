...
public class Tournament {
	...
	String champions;
        ...
        public Tournament(String Name) throws Exception{
            ...
            while (jornadasSC.hasNextLine()){
                ...
                */
                ...
                if(inputs[6] != null){
                    teams.get(homeTeam).getRoster().get(inputs[6]).setGoals(1);
                }
                
                if(inputs[7].length()> 0){
                    teams.get(homeTeam).getRoster().get(inputs[7]).setGoals(1);
                }
        ...
        public void showStandings(){
            ...
            List<Team> teamsForStandings = new ArrayList<Team>(teams.values());
            
            //sort teams using a lambda
            teamsForStandings.sort((Team t1, Team t2)-> t2.getPoints().compareTo(t1.getPoints()));
            for(Team tt : teamsForStandings) {
        ...
        public Collection<Player> getPlayersByTeam(String teamName){
            for(Player p : teams.get(teamName).getRoster().values()){
                System.out.println(p.toString());
            }
            return teams.get(teamName).getRoster().values();
        }
